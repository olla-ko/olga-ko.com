...
```bash
npm test        # весь набор, меньше секунды
```

Встроенный раннер Node (`node:test`) — **никаких тестовых библиотек в зависимостях**.

| Где | Что проверяет |
|---|---|
| [`test/bake.test.js`](test/bake.test.js) | [`lib/bake.js`](lib/bake.js) на фикстурах из `test/fixtures/`: карточки портфолио, записи резюме, якоря заголовков, ручные размеры картинок |
| [`test/build.test.js`](test/build.test.js) | инварианты собранного сайта: все разделы собираются и непусты, корень редиректит, у страниц есть `<title>` и `lang`, не осталось неразобранных `<<Тегов>>` |
| [`admin/test/*.test.js`](admin/test/) | логика админки: правила сохранения, лента версий, бейдж черновика, гарды удаления |

Логика админки живёт в [`admin/lib/`](admin/lib/) отдельно от `admin/server.js`:
это **чистые функции без зависимостей** — ни сети, ни файлов, ни Express. Сервер
остаётся транспортом. Правило простое: если появляется новое правило принятия
решений, оно пишется в `admin/lib/`, а не в обработчик — иначе его нельзя
проверить иначе как руками.

Сборку `test/build.test.js` запускает программно (`toJSON()`), а не командой:
обычная сборка копирует ~280 МБ архива блога, а программная даёт те же страницы
за доли секунды и ничего не пишет на диск.

### Главное правило

> **Нашли баг → сначала тест, который его воспроизводит (красный) → потом починка (зелёный).**

Так каждый исправленный дефект остаётся закрытым навсегда. Все известные баги
админки уже зафиксированы такими тестами — потеря правок при одновременном
сохранении, призрачные версии в ленте, ложный бейдж «не опубликовано», потеря
истории при удалении версии.

Тесты на код, написанный раньше, — *характеризующие*: они фиксируют поведение
как есть. Если такой тест проходит сразу, это ещё ничего не доказывает — стоит
нарочно сломать реализацию и убедиться, что тест краснеет.

### Тесты в CI

Job `test` в [pages.yml](.github/workflows/pages.yml) идёт перед деплоем: красный
тест останавливает публикацию. Коммиты, меняющие **только контент** (`content/*.md`
и `*/content.md` — ровно то, что пишет админка), тесты пропускают, чтобы публикация
из редактора не зависела от состояния кода.
